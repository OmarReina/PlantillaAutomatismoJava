package co.com.claro.nameproject.ws.soap.client;
            ...
            try {
                ...
                for (Operation operation : wsdlInterface.getOperationList()) {
                    ...
                    if (wsdlOperation.getName().equals(metodo)) {
                        ...
                        ClaroLogger.infoProgrammerLog(String.format(Log4jConstans.LOG4J_WSDL, Util.toJson(wsdl)));
                        ClaroLogger.infoProgrammerLog(String.format(Log4jConstans.LOG4J_NAME_WS, Util.toJson(nameWS)));
                        ClaroLogger.infoProgrammerLog(String.format(Log4jConstans.LOG4J_METODO, Util.toJson(metodo)));
                        ClaroLogger.infoProgrammerLog(String.format(Log4jConstans.LOG4J_TIMEOUT, Util.toJson(timeOut)));
                        ClaroLogger.infoProgrammerLog(String.format(Log4jConstans.LOG4J_REQUEST, StringEscapeUtils.unescapeJava(wsdlRequest.getRequestContent().replaceAll("\\n|\\r", ""))));
                ...
                if (response != null) {
                    ClaroLogger.infoProgrammerLog(String.format(Log4jConstans.LOG4J_RESPONSE, StringEscapeUtils.unescapeJava(response).replaceAll("\\n|\\r", "")));
                    if (response == null || !response.isEmpty()) {
                        switch (metodo) {
                            case ServiceSoapConstans.MAXIMO_METHOD_WS_CREATE_INCIDENT:
                                response = Util.extractTextBetweenStrings(response, "<TICKETID>", "</TICKETID>");
                                break;
                            case ServiceSoapConstans.MAXIMO_METHOD_WS_CREATE_OT:
                                response = Util.extractTextBetweenStrings(response, "<WONUM>", "</WONUM>");
                                break;
                            case ServiceSoapConstans.MAXIMO_METHOD_WS_CREATE_TASK:
                                response = Util.extractTextBetweenStrings(response, "<WONUM>", "</WONUM>");
                                break;
                        }
                    }
